...
//Makes a brief calculation side by side without considering the last one with the first on (closure)
...
//                                    GeoElement *elementv2 = vecinos[i][2][1].fElement;
    ...
    for(int i=0;i<NumEl;i++){
        std::cout<<"Neighbours of element: "<<i<<"\n";
        GeoElement *elementi = Element(i);
...
void GeoMesh::Print(std::ostream &out){
    ...
    out << "Nodes number = " << Nodes.size() << std::endl;
    for (i=0;i<Nodes.size();i++)
    {
        out << "Index " << i << ' ';
        Nodes[i].Print(out);
    }
    out <<"\n" <<"Element number = " << Elements.size() << std::endl;
